...
BOOL CProject::LoadPropMoverEx_AI_SCAN( LPCTSTR szFileName, CScript &script, int nVal )
{
	...
	while(1)
	{
		...
		if( script.tokenType == IDENTIFIER )		// 토큰이 식별자.
		{
			...
			{
				if( _strcmpi( script.token, "job" ) == 0 )		// 스캔 직업 
					pProp->m_nScanJob = script.GetNumber();
				else 
				if( _strcmpi( script.token, "range" ) == 0 )		// 스캔 범위 
					pProp->m_nAttackFirstRange = script.GetNumber();
				else 
				if( _strcmpi( script.token, "quest" ) == 0 )	// 퀘스트를 갖은 플레이어 스켄 
					pProp->m_dwScanQuestId = script.GetNumber();
				else 
				if( _strcmpi( script.token, "item" ) == 0 )	// 해당 아이템을 갖은 플레이어 스켄 
					pProp->m_dwScanItemIdx = script.GetNumber();
				else
				if( _strcmpi( script.token, "chao" ) == 0 )	// 해당 아이템을 갖은 플레이어 스켄 
			...
			if( _strcmpi( script.Token, "scan" ) == 0 )	// 명령어.
			{
...
BOOL CProject::LoadPropMoverEx_AI_BATTLE( LPCTSTR szFileName, CScript &script, int nVal )
{
	...
	while(1)
	{
		...
		{
			//---- Attack
			if( _strcmpi( script.token, "Attack" ) == 0 )	// 일반 공격명령.
			{
				nCommand =	AICMD_ATTACK;	//	명령 "공격" 시작.
				pProp->m_bMeleeAttack = 1;	// 근접공격 AI도 있다는것.
			} else
			//--- cunning
			if( _strcmpi( script.token, "cunning" ) == 0 )	// 공격명령시 레벨 비교
			{
				...
				{
					script.GetToken();
					if( _strcmpi( script.token, "low" ) == 0 )		// 타겟이 레벨이 낮은경우 공격
						pProp->m_nLvCond = 1;
					else if( _strcmpi( script.token, "Sam" ) == 0 )	// 타겟이 레벨이 같은경우.
						pProp->m_nLvCond = 2;
					else if( _strcmpi( script.token, "Hi" ) == 0 )	// 타겟이 레벨이 높은경우.
						pProp->m_nLvCond = 3;
					else
					{
						SYNTAX_ERROR;
			...
			//--- Recovery
			if( _strcmpi( script.token, "Recovery" ) == 0 )	// 회복 명령.
			{
			...
			if( _strcmpi( script.token, "u" ) == 0 || 
				_strcmpi( script.token, "m" ) == 0 || 
				_strcmpi( script.token, "a" ) == 0 )	// 대상설정
			{
				...
				{
//					script.GetToken();
					if( _strcmpi( script.token, "u" ) == 0 )		// 대상은 다른이.
						pProp->m_bRecvCondWho = 1;
					else if( _strcmpi( script.token, "m" ) == 0 )	// 대상은 나
						pProp->m_bRecvCondWho = 2;
					else if( _strcmpi( script.token, "a" ) == 0 )	// 대상은 다른이,나
						pProp->m_bRecvCondWho = 3;

					pProp->m_bRecvCond = 2;			// 일단 옵션이 붙었으므로 전투/비전투 모두 힐링.
				}
			} else
			if( _strcmpi( script.token, "RangeAttack" ) == 0 )	// 원거리 공격
			{
				nCommand = AICMD_RANGEATTACK;
			} else
			if( _strcmpi( script.token, "KeepRangeAttack" ) == 0 )	// 거리유지 원거리 공격
			{
				nCommand = AICMD_KEEP_RANGEATTACK;
			} else
			if( _strcmpi( script.token, "Summon" ) == 0 )		// 소환 명령
			{
				nCommand = AICMD_SUMMON;
			} else
			if( _strcmpi( script.token, "Evade" ) == 0 )			// 도망
			{
				nCommand = AICMD_EVADE;
			} else
			if( _strcmpi( script.token, "Helper" ) == 0 )		// 도움요청.
			{
			...
			// helper에서 쓰이는..
			if( _strcmpi( script.token, "all" ) == 0 ||
				_strcmpi( script.token, "sam" ) == 0 )
			{
				...
				{
					if( _strcmpi( script.token, "all" ) == 0 )			// 아무나.
						pProp->m_bHelpWho = 1;	
					else if( _strcmpi( script.token, "sam" ) == 0 )		// 같은 종만
						pProp->m_bHelpWho = 2;
				}
			} else
			if( _strcmpi( script.token, "Berserk" ) == 0 )		// 광포화.
			{
				nCommand = AICMD_BERSERK;
			} else
			if( _strcmpi( script.token, "Randomtarget" ) == 0 )	
			{
			} else
...
BOOL CProject::LoadPropMoverEx_AI_MOVE( LPCTSTR szFileName, CScript &script, int nVal )
{
	...
	while(1)
	{
		...
		if( script.tokenType == IDENTIFIER )		// 토큰이 식별자.
		{
			if( _strcmpi( script.token, "Loot" ) == 0 )		// 루팅. 아이템 줏어먹기.
			{
				nCommand = AICMD_LOOT;
//				pProp->m_nLoot = 1;		// 康: 임시로 막음
				pProp->m_nLoot	= 0;
			} else
			if( _strcmpi( script.token, "d" ) == 0 )		// Loot명령 옵션
			{
